...
- push_priority: 这篇文章对 Agent 领域整体的重要程度（不是对某个具体项目的影响）
  * "high"   — 模型能力跃迁 / 协议标准变动 / 新范式 / 被广泛采用的基础设施
  * "medium" — 值得关注的新框架/工具 / 有趣的实验 / 明显趋势信号
  * "low"    — 渐进改进 / 小项目 / 已知信息的重复
- 只输出 JSON，不要其他内容
...
def summarize(articles: list[dict]) -> list[dict]:
    ...
    for a in articles:
        ...
        if match:
            ...
            item["push_priority"] = match.get("push_priority", "low")
        else:
            item["domain"] = "其他"
            item["core_content"] = ""
            item["value_judgment"] = ""
            item["push_priority"] = "low"
        merged.append(item)
